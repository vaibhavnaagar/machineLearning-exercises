...
% recode y of mx1 to Y mxK i.e y_i = [ 0 0 0 ...(ith)1....0]'
...
%----------Gradient---------------
del3 = a3 - Y;                                                                  % (m x 10)
Theta2_grad = (del3'*[ones(m, 1) a2])/m + Theta2*lambda/m;                      % (10 x 26)
Theta2_grad(:, 1) = Theta2_grad(:, 1) - Theta2(:, 1)*lambda/m;

del2 = ((del3*Theta2)(:, 2:end)).*sigmoidGradient([ones(m, 1) X]*Theta1');      % (m x 25)
Theta1_grad = (del2'*[ones(m, 1) X])/m + Theta1*lambda/m;                       % (25 x 401)
